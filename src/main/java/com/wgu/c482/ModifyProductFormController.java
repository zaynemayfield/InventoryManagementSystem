...
/**
 * The ModifyProductFormController class handles all the operations with ModifyProductForm.fxml
 * */
public class ModifyProductFormController implements Initializable {
    ...
    private Parent root;
    /**
     * @param modifyProduct gets the product and fills out the details in the text fields
     * */
    public void getProduct(Product modifyProduct) {
        this.product = modifyProduct;
        productID.setText(String.valueOf(product.getId()));
        productName.setText(product.getName());
        productInventory.setText(String.valueOf(product.getStock()));
        productCost.setText(String.valueOf(product.getPrice()));
        productMax.setText(String.valueOf(product.getMax()));
        productMin.setText(String.valueOf(product.getMin()));
        associatedParts = modifyProduct.getAssociatedParts();
        associatedPartTable.setItems(modifyProduct.getAssociatedParts());

    }
    /**
     * @param actionEvent Grabs selected part and adds to the product, if nothing selected then updates status.
     * */
    ...
    /**
     * @param actionEvent Grabs selected part and removes from the product, if nothing selected then updates status.
     *                    Changed from deleting the associated part from here to using the product method to do it.
     * */
    public void removePartFromProduct(ActionEvent actionEvent) {
        ...
        if (this.product.deleteAssociatedPart(deletePart)) {
    ...
    /**
     * @param actionEvent this checks to make sure the text fields are not null.
     *                    If one is then it displays a message letting the user know.
     *                    I had this checking to see if the product had no parts associated with it, but then had to change this because of the requirement that allows this.
...
//        }
    ...
    /**
     * This checks if the inventory, max, min are all correct.
     * */
    ...
    /**
     * @param actionEvent this even saves the modified product.
     *                    I originally had this deleting the product and generating a new one, but changed it to use the setters in the product model class.
     * */
    public void saveProduct(ActionEvent actionEvent) throws IOException {
        this.product.setName(productName.getText());
        this.product.setPrice(Double.parseDouble(productCost.getText()));
        this.product.setStock(Integer.parseInt(productInventory.getText()));
        this.product.setMin(Integer.parseInt(productMin.getText()));
        this.product.setMax(Integer.parseInt(productMax.getText()));
        goToMain(actionEvent);

    }
    /**
    * @param actionEvent cancels and goes to main form
    * */
    ...
    /**
     * @param actionEvent goes to main form
     * */
    ...
    /**
     * Fills out the data for the tables.
     * */
    @Override
